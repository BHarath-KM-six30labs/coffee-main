...
export function InventoryView() {
  ...
  const totalCoffeeIn = items.reduce((sum, item) => sum + item.totalCoffeeIn, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-xl font-semibold text-[#4A2C2A] mb-4">Inventory by Category</h2>
                ...
                <tr key={category} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      weight < 1000 ? 'bg-red-100 text-red-800' :
                      weight < 5000 ? 'bg-yellow-100 text-yellow-800' :
                      'bg-green-100 text-green-800'
                    }`}>
                      {weight < 1000 ? 'Low Stock' :
                       weight < 5000 ? 'Moderate' :
                       'Good Stock'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-xl font-semibold text-[#4A2C2A] mb-4">Processing Services Overview</h2>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">% of Total Volume</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
